...
test("outreach CLI emits a maintainer-friendly issue draft", async () => {
  const { stdout } = await execFileAsync(process.execPath, [bin, "outreach", "--root", fixture("empty-repo")], { cwd: root });

  assert.match(stdout, /Agent readiness suggestion for empty-repo/);
  assert.match(stdout, /A local dry run suggests/);
  assert.match(stdout, /npx --yes @eshen_fox_mie\/agent-ready improve --preset team --dry-run --format issue/);
  assert.match(stdout, /If this is not useful, feel free to close this/);
  assert.doesNotMatch(stdout, /Please star/);
});
...
test("ci CLI emits reusable GitHub Action workflow by default", async () => {
  ...
  assert.match(stdout, /uses: EShener\/agent-ready@v0\.1\.25/);
